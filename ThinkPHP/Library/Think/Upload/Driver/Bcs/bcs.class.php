...
class BaiduBCS {
	...
	//ACL_TYPE:
	//公开读权限
	const BCS_SDK_ACL_TYPE_PUBLIC_READ = "PublicABC-read";
	//公开写权限（不具备删除权限）
	const BCS_SDK_ACL_TYPE_PUBLIC_WRITE = "PublicABC-write";
	//公开读写权限（不具备删除权限）
	const BCS_SDK_ACL_TYPE_PUBLIC_READ_WRITE = "PublicABC-read-write";
	//公开所有权限
	const BCS_SDK_ACL_TYPE_PUBLIC_CONTROL = "PublicABC-control";
	...
	private function authenticate($opt) {
		...
		if (isset ( $opt ['fileUpload'] )) {
			...
			$length = $request->read_stream_size; // The file size by Person
			$file_size = $length;
			if (isset ( $opt ["length"] )) {
			...
			if (isset ( $opt ['seekTo'] ) && ! isset ( $opt ["length"] )) {
				// Read from seekTo until EOF by Person, when set seekTo but not set $opt["length"]
	...
	public function create_object_superfile($bucket, $object, $file, $opt = array()) {
		...
		$sub_object_size = 1024 * 1024; //Person 1MB
	 ...
	 * boolean has_sub_directory(Person=true)   object命名中是否携带文件的子目录结构，若置为false，请确认待上传的目录和所有子目录中没有重名文件，否则会产生object覆盖问题
