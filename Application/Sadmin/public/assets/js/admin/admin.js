...
function submit_login(){
  ...
  //if($('input[name="rememberMe"]').prop("checked")){ //如果选中密码记住我
  //    var rememberMe = "yes";
  //
  //}
    //alert(rememberMe);
  ...
  if(loginUsername==""||loginPassword==""){   //当用户名或密码为空时，提醒用户
    ...
    $.ajax({//不为空，到后台去验证用户名和密码是否正确
          url: admin_usernamePassword_url, //管理员用户名检测地址
          type: 'post',
          dataType: 'json',
          data:{loginUsername:loginUsername,loginPassword:loginPassword},
          success:function(result){
            if(result.existed=="yes"){ //用户名存在时
                // alert(result.sessionUsername);
                // alert(result.cookieUsername);
                // alert(result.cookiePassword);
                window.location.href= index_url;
                // $("#loginUsername").val(result.cookieUsername);
                // $("#loginPassword").val(result.cookiePassword);
            }else if(result.existed=="no"){ //用户名不存在时
                layer.confirm('用户名或密码不正确或账户已被停用', {
                 btn: ['确定'] //按钮
                });
                return false;
